...
sync
reboot
apk update && apk add e2fsprogs-tune2fs  # OpenWrt/ImmortalWrt 25.x
# Or on older releases: opkg update && opkg install tune2fs
tune2fs -m 1 "$(readlink -f /dev/root)"
```
**Optional: Clean up**
```bash
...
reboot
apk update && apk add e2fsprogs-tune2fs  # OpenWrt/ImmortalWrt 25.x
# 旧版本可使用：opkg update && opkg install tune2fs
tune2fs -m 1 "$(readlink -f /dev/root)"
```
**可选：清理残留**
```bash
